    ...
    <main>
        <div class="category_heading">
            <?php
                ...
                $row_cat = mysqli_fetch_assoc($sql1);
            ?>
            <h3><?php echo $row_cat["category_name"]; ?></h3>
        </div>
                <?php
        include "configure.php";
        if (isset($_GET["pages"])) {
        ...
        ?>
                <div class="card_container">
                    <a href="single_page.php?id=<?php echo $row["post_id"]; ?>" style="text-decoration: none;"><div class="card_element">
                        <div class="image">
        ...
        <?php
                if (mysqli_num_rows($sql1) > 0) {
                    // $page = $_GET["page"];
                    $total_record = $row_cat["post"];
                    // $limit = 5;
                    $total_pages = ceil($total_record / $limit);
                    // $offset =($page-1)*$limit;
                    echo '<ul class="pagination">';
                    if ($page > 1) {
                        echo '<li><a href="category.php?cid='.$cat_id.'&pages=' . ($page - 1) . '">&laquo; PREV</a></li>';
                    }
                    for ($i = 1; $i <= $total_pages; $i++) {
                        ...
                        echo '<li><a href="category.php?cid='.$cat_id.'&pages=' . $i . '" class="' . $active . '">' . $i . '</a></li>';
                    }
                    if ($total_pages > $page) {
                        echo '<li><a href="category.php?cid='.$cat_id.'&pages=' . ($page + 1) . '">NEXT &raquo;</a></li>';
                    }
                    echo '</ul>';
                }
                ?>
