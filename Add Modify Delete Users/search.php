...
<body>
    ...
    <main>
        <div class="category_heading">
            <h3>Search : <?php echo $search_term; ?></h3>
        </div>
                <?php
        include "configure.php";
        if (isset($_GET["pages"])) {
        ...
        ?>
        <?php
                $query_1 = "select * from post
                where post.title like '%{$search_term}%'";
                $sql_1 = mysqli_query($connection,$query_1) or die("querry fail 1");
                if (mysqli_num_rows($sql_1) > 0) {
                    // $page = $_GET["page"];
                    $total_record = mysqli_num_rows($sql_1);
                    // $limit = 5;
                    $total_pages = ceil($total_record / $limit);
                    // $offset =($page-1)*$limit;
                    echo '<ul class="pagination">';
                    if ($page > 1) {
                        echo '<li><a href="search.php?search='.$search_term.'&pages=' . ($page - 1) . '">&laquo; PREV</a></li>';
                    }
                    for ($i = 1; $i <= $total_pages; $i++) {
                        ...
                        echo '<li><a href="search.php?search='.$search_term.'&pages=' . $i . '" class="' . $active . '">' . $i . '</a></li>';
                    }
                    if ($total_pages > $page) {
                        echo '<li><a href="search.php?search='.$search_term.'&pages=' . ($page + 1) . '">NEXT &raquo;</a></li>';
                    }
                    echo '</ul>';
                }
                ?>
